...
class SoundManager {
  ...
  playBackgroundMusic() {
    ...
    this.bgmOscillator.frequency.setValueAtTime(110, now);
    
    // 펄싱 음악 효과
    const pulseDuration = 1;
    let time = now;
    for (let i = 0; i < 100; i++) {
      const freq = 110 + Math.sin(i * 0.3) * 30;
      this.bgmOscillator.frequency.setTargetAtTime(freq, time, 0.1);
      time += 0.1;
    }
    
    this.bgmGain.gain.setValueAtTime(this.masterVolume * 0.15, now);
  ...
  playBossDefeatSound() {
    ...
    for (let i = 0; i < 4; i++) {
      setTimeout(() => {
        ...
        osc.frequency.setValueAtTime(1200 + i * 150, currentTime);
...
class GameManager {
  constructor() {
    ...
    this.state = 'intro'; // 🔴 'intro' 상태에서 시작
    this.introTimer = 0;
    this.introPhase = 0; // 0: 타이틀, 1: 설명, 2: 조작법, 3: 시작 대기
    ...
    // 🔴 인트로 음악 재생
    this.soundManager.playIntroSound();
